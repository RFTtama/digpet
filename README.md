...
## Webカメラを使用した機能の使用方法
通常はDigpetが実行されているコンピュータのCPU使用率を使用して、感情の計算を行います。
Web Cameraを使用することで、インタラクティブに計算を行うこともできます。
この場合顔検出の有無で感情が計算されます。
(DigpetがWeb Cameraから取得した画像は顔検出処理後に破棄され、画像を無断で保存したり、外部に送信することは決してありません。)
1. アプリを終了します。(起動中の場合)
