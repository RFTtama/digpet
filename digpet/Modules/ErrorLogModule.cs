    ...
    static class ErrorLogModule
    {
        ...
        public static void ErrorOutput(string name, string msg, bool show)
        {
            try
            {
                ...
                if (show) TaskMessageModule.OutputMessage(msg, name, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (Exception ex)
            {
                TaskMessageModule.OutputMessage(ex.Message, "エラー出力エラー");
            }
        }
