...
namespace digpet.Managers
{
    internal class CharZipFileManager
    {
        ...
        public void ReadCharSettings(string path)
        {
            if (!File.Exists(path))
            {
                ErrorLogModule.ErrorOutput("コンフィグファイル確認エラー", "キャラデータが見つかりません");
                return;
            }
            ...
            try
            {
                using (ZipArchive zip = ZipFile.OpenRead(path))
                {
                    ...
                    if (entry == null)
                    {
                        ErrorLogModule.ErrorOutput("コンフィグファイル読み取りエラー", "キャラデータにコンフィグファイルが含まれていません");
                        return;
                    }
            ...
            {
                ErrorLogModule.ErrorOutput("コンフィグファイル読み取りエラー", ex.Message);
            }
        }
        ...
        private bool IsHandleVersion()
        {
            ...
            if (charVersion.major != -1 && availableVersion.major != -1)
            {
                ...
                {
                    ErrorLogModule.ErrorOutput("キャラファイルバージョンエラー", "キャラファイルのバージョンがサポートされている最大のバージョン("
                        + SettingManager.PrivateSettings.CHAR_FORMAT_VERSION + ")より大きいです");
                }
            }
            else
            {
                ErrorLogModule.ErrorOutput("キャラファイルバージョンエラー", "バージョン情報が正常に設定されませんでした");
            }
        ...
        private void SetImageList(ZipArchive zip)
        {
            ...
            {
                ErrorLogModule.ErrorOutput("イメージ読み取りエラー", ex.Message);
            }
        }
        ...
        private class CharSettingManager
        {
            ...
            public int ReadEntry(ZipArchiveEntry entry)
            {
                ...
                try
                {
                    ...
                    {
                        LogModule.LogOutput("キャラファイルのコンフィグデータ読み込みに失敗しました");
                        ErrorLogModule.ErrorOutput("コンフィグ読み取りエラー", "コンフィグデータがNULLです");
                    }
                    else if (string.IsNullOrEmpty(settings_tmp.charSettings.name))
                    {
                        LogModule.LogOutput("設定ファイルが正しく読み取られませんでした");
                        ErrorLogModule.ErrorOutput("コンフィグ読み取りエラー", "キャラファイルのコンフィグデータが正しく設定されていない可能性があります");
                    }
                    else
                    {
                        LogModule.LogOutput("キャラファイルのコンフィグデータが正常に読み込まれました");
                ...
                {
                    LogModule.LogOutput("キャラファイルのコンフィグデータ読み込みに失敗しました");
                    ErrorLogModule.ErrorOutput("コンフィグ読み取りエラー", ex.Message);
                }
